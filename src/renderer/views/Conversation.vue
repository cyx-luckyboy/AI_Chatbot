<template>
  <div class="relative flex min-h-0 flex-1 flex-col overflow-hidden bg-slate-100 dark:bg-slate-950">
    ...
    >
    <div
      class="flex shrink-0 items-center justify-between gap-3 border-b border-slate-200 bg-slate-200/90 px-3 py-2 dark:border-slate-600 dark:bg-slate-900"
      v-if="conversationDisplay"
    >
      <div class="min-w-0 flex-1">
        <h3 class="truncate font-semibold text-gray-900 dark:text-slate-100">{{ conversationDisplay.title }}</h3>
        <span class="text-xs text-gray-500 dark:text-slate-400">{{ formatDateTime(conversationDisplay.updatedAt) }}</span>
      </div>
      <div class="w-[min(100%,16rem)] shrink-0">
        <ProviderSelect
          :items="providers"
          v-model="conversationProviderModel"
    ...
    <div class="relative z-20 shrink-0 flex items-center border-t border-slate-200 bg-slate-100 py-2 dark:border-slate-800 dark:bg-slate-950">
      <div class="mx-auto w-[80%]">
        <MessageInput
          ref="messageInputRef"
...
import { isActiveChatReply, isPendingImageGenerationAnswer, useMessageStore } from '../../domains/chat/stores/message'
...
import { DEFAULT_JIMENG_IMAGE_MODEL, resolveJimengImageModelId } from '../../domains/media/jimengModels'
...
import { buildTranslatePromptForModel } from '../../domains/media/translatePrompt'
...
} from '../../domains/media/ppt/pptChatMessages'
import { withNailongPersona } from '../../domains/pet/petChatHandlers'
import { pptScenarioUsesTwoStage } from '../../domains/media/ppt/pptScenarioPrompts'
import { looksLikeImageGenerationPrompt } from '../../domains/media/imageGenPromptDetect'
import { db } from '../../shared/db'
import type { ConversationProps } from '../../shared/types'
import { setMeetingReturnContext } from '../../domains/meeting/meetingChatPost'
...
const providers = computed(() => provdierStore.items)

const conversationProviderModel = computed({
  get() {
    const conv = conversationDisplay.value
    if (!conv) return ''
    return `${conv.providerId}/${conv.selectedModel}`
  },
  set(value: string) {
    ...
    if (!Number.isFinite(providerId) || !selectedModel) return
    if (isReplyInProgress.value) return
    void conversationStore.updateConversationModel(conversationId.value, providerId, selectedModel)
  },
})

async function sealStuckStreams(sealedIds: number[]) {
  for (const id of sealedIds) {
    staleStreamMessageIds.add(id)
    void window.electronAPI.petForceIdle({ messageId: id })
  }
}
...
const isReplyInProgress = computed(() => {
  ...
  return messageStore.items.some(
    (item) => item.conversationId === cid && isActiveChatReply(item),
  )
})

async function onStopGenerating() {
  const active = messageStore.items.find(
    (item) => item.conversationId === conversationId.value && isActiveChatReply(item),
  )
  try {
    await window.electronAPI.abortChat(
      active?.id != null ? { messageId: active.id } : undefined,
    )
  } catch (e) {
    console.error('abortChat', e)
  }
  // 立刻解锁输入框；主进程稍后会再推一帧结束内容
  ...
  for (const item of messageStore.items) {
    ...
    staleStreamMessageIds.add(item.id)
  }
}
...
const petConversationIdRef = ref<number | null>(null)
void window.electronAPI.getConfig().then((cfg) => {
  const id = Number(cfg.petConversationId)
  petConversationIdRef.value = Number.isFinite(id) && id > 0 ? id : null
})
...
const sendedMessages = computed(() => {
  const base = buildChatMessagesForConversation(
    filteredMessages.value,
    locale.value === 'zh' ? 'zh' : 'en',
    pptChatOverride.value ?? undefined,
  )
  const isPet =
    (petConversationIdRef.value != null && conversationId.value === petConversationIdRef.value) ||
    conversationDisplay.value?.title === t('settings.petChatConversationTitle')
  return isPet ? withNailongPersona(base) : base
})
...
async function openMeetings() {
  const conv = conversationDisplay.value
  if (!conv) return
  const sealed = await messageStore.fetchMessagesByConversation(conv.id)
  await sealStuckStreams(sealed)
  const provider = `${conv.providerId}/${conv.selectedModel}`
  setMeetingReturnContext(conv.id, provider)
  router.push({
    path: '/meetings',
    query: {
...
const sendNewMessage = async (payload: MessageCreatePayload) => {
  ...
  const questionId = await messageStore.createMessage({
    ...
    ...(payload.agentWithModel && { agentMode: true }),
  })
  ...
  const answerId = await creatingInitialMessage({
    webSearch: Boolean(payload.webSearchWithModel),
    agentMode: Boolean(payload.agentWithModel),
  })
  if (payload.pptGenerateWithModel && pptScenarioUsesTwoStage(pptScenario)) {
...
const onRegenerateAnswer = async (answerMessageId: number) => {
  ...
  const answerId = await creatingInitialMessage({
    webSearch: Boolean(q?.webSearch),
    agentMode: Boolean(q?.agentMode),
  })
...
function resolveAgentModeForChat(opts?: { forAnswerId?: number; agentMode?: boolean }): boolean {
  if (opts?.agentMode !== undefined) return opts.agentMode
  const q =
    opts?.forAnswerId != null
      ? findQuestionForAnswer(opts.forAnswerId)
      : messageStore.getLastQuestion(conversationId.value)
  return Boolean(q?.agentMode)
}
...
const creatingInitialMessage = async (opts?: {
  ...
  const conv =
    ...
    (await db.conversations.get(conversationId.value))
  if (!conv) {
    await messageStore.updateMessage(newMessageId, {
      content: t('settings.petChatNoReadyProvider'),
      status: 'error',
      updatedAt: new Date().toISOString(),
    })
    void window.electronAPI.petForceIdle({ messageId: newMessageId })
    return newMessageId
  }
  const provider = provdierStore.getProviderById(conv.providerId)
  if (!provider) {
  ...
  await window.electronAPI.startChat(
    serializeCreateChatProps({
      ...
      agentMode: resolveAgentModeForChat(opts),
...
/** 离开对话页时已结束的流式回复；主进程仍可能推送尾随帧，需忽略以免输入框再次被禁用 */
const staleStreamMessageIds = new Set<number>()
...
watch(
  () => filteredMessages.value.filter((m) => m.meetingId).length,
  async (n, prev) => {
    if (n > prev) {
      await nextTick()
      await messageScrollToBottom()
...
watch(() => route.params.id, async (newId: string) => {
  streamBuffers.clear()
  staleStreamMessageIds.clear()
  pptChatOverride.value = null
  pptPipelinePending.value = null
  conversationId.value = parseInt(newId)
  const sealed = await messageStore.fetchMessagesByConversation(conversationId.value)
  await sealStuckStreams(sealed)
  await messageScrollToBottom()
...
onMounted(async () => {
  ...
  if (provdierStore.items.length === 0) {
    await provdierStore.fetchProviders()
  }
  const sealed = await messageStore.fetchMessagesByConversation(conversationId.value)
  await sealStuckStreams(sealed)
  await messageScrollToBottom()
  await nextTick(() => messageInputRef.value?.focusInput())
  if (initMessageId) {
  ...
  removeUpdateListener = window.electronAPI.onUpdateMessage((streamData) => {
    streamApplyChain = streamApplyChain.then(async () => {
      ...
      if (staleStreamMessageIds.has(messageId)) return
      const streamMsgEarly = messageStore.items.find((m) => m.id === messageId)
      if (streamMsgEarly?.meetingId != null) return
