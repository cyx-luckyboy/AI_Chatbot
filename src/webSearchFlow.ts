...
import { enrichSearchQueryWithLocation } from './userLocation'
...
function sendChunk(win: BrowserWindow, messageId: number, data: UpdatgedStreamData['data']) {
...
export async function runWebSearchChatPipeline(
  ...
  if (!userQuestion) {
    await streamChatToWindow(win, provider, messages, selectedModel, messageId)
    return
  }
  ...
  if (!ok) {
    sendChunk(win, messageId, {
      ...
      result:
        lang === 'zh'
          ? '联网检索已完成，但对话模型未返回任何文字。请检查模型是否可用，或更换模型后重试。'
          : 'Search completed but the chat model returned no text. Check the model or try another one.',
      is_error: true,
      replace: true,
    })
  }
