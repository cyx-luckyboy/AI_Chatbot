<template>
  <div class="message-list" ref="_ref">
    ...
    <div
      v-if="batchMode"
      class="sticky top-0 z-10 mb-2 flex items-center justify-between gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 shadow-sm dark:border-red-900/50 dark:bg-red-950/40"
    >
      <div class="flex items-center gap-2">
        <span class="text-sm font-medium text-red-700 dark:text-red-300">
          {{ t('common.messageBatchDeleteSelected', { count: batchSelectedIds.size }) }}
        </span>
        <button
          type="button"
        ...
        >
          {{ batchSelectedIds.size === messages.length ? t('common.messageBatchDeleteDeselectAll') : t('common.messageBatchDeleteSelectAll') }}
        </button>
      </div>
      <div class="flex items-center gap-2">
        <button
          type="button"
          class="rounded-md border border-red-300 bg-white px-3 py-1 text-xs font-medium text-red-600 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-red-700 dark:bg-transparent dark:text-red-400 dark:hover:bg-red-950/30"
          :disabled="batchSelectedIds.size === 0"
          @click="confirmBatchDelete"
        >
          {{ t('common.messageBatchDeleteConfirmBtn') }}（{{ batchSelectedIds.size }}）
        </button>
        <button
          type="button"
          class="rounded-md border border-gray-300 bg-white px-3 py-1 text-xs font-medium text-gray-600 hover:bg-gray-50 dark:border-slate-600 dark:bg-transparent dark:text-slate-300 dark:hover:bg-slate-800"
          @click="exitBatchMode"
        >
          {{ t('common.messageBatchDeleteExit') }}
        </button>
      </div>
    </div>
    <div class="message-item mb-3" v-for="message in messages" :key="message.id">
      <div class="flex" :class="{'justify-end': message.type === 'question' && !batchMode, 'items-start gap-2': batchMode}">
        <!-- 批量选择复选框 -->
        <button
          v-if="batchMode"
          type="button"
          class="mt-7 flex shrink-0 items-center justify-center"
          @click="toggleSelect(message.id)"
        >
          <span
            class="flex h-5 w-5 items-center justify-center rounded border-2 transition-colors"
            :class="
              batchSelectedIds.has(message.id)
                ? 'border-blue-500 bg-blue-500 dark:border-blue-400 dark:bg-blue-400'
                : 'border-gray-400 bg-white dark:border-slate-500 dark:bg-slate-800'
            "
          >
            <Icon
              v-if="batchSelectedIds.has(message.id)"
              icon="mdi:check"
              width="14"
              height="14"
              class="text-white"
        ...
        <div :class="batchMode ? 'min-w-0 flex-1' : ''">
          <div class="mb-2 text-sm text-gray-500 dark:text-slate-400" :class="{'text-right': message.type === 'question' && !batchMode}">
            {{ formatDateTime(message.createdAt) }}
          </div>
          <div
            v-if="message.type === 'question'"
            class="flex w-full flex-col gap-1"
            :class="batchMode ? 'items-start' : 'items-end'"
          >
            <div
            ...
            >
              ...
              <span
                v-if="message.agentMode"
                class="mb-1 inline-flex items-center gap-0.5 rounded bg-white/15 px-1.5 py-0.5 text-[11px] text-white/90"
              >
                <Icon icon="mdi:desktop-classic" width="12" height="12" />
                {{ t('common.agentTag') }}
              </span>
            ...
            <div
              ...
              :class="batchMode ? 'justify-start' : 'justify-end'"
            >
              <DropdownMenuRoot
                v-if="!batchMode"
              ...
              <button
                v-if="!batchMode"
                type="button"
                class="inline-flex h-8 w-8 items-center justify-center rounded-md text-gray-400 outline-none hover:bg-red-50 hover:text-red-500 focus-visible:ring-2 focus-visible:ring-red-400 dark:text-slate-500 dark:hover:bg-red-950/30 dark:hover:text-red-400"
                :title="t('common.messageDelete')"
                :aria-label="t('common.messageDelete')"
                @click="onDeleteMessage(message.id)"
              >
                <Icon icon="radix-icons:trash" class="h-[18px] w-[18px]" />
              </button>
              <!-- 进入批量删除 -->
              <button
                v-if="!batchMode"
                type="button"
                class="inline-flex h-8 w-8 items-center justify-center rounded-md text-gray-400 outline-none hover:bg-gray-200/90 hover:text-gray-700 focus-visible:ring-2 focus-visible:ring-gray-400 dark:text-slate-500 dark:hover:bg-slate-700 dark:hover:text-slate-300"
                :title="t('common.messageBatchDelete')"
                :aria-label="t('common.messageBatchDelete')"
                @click="enterBatchMode(message.id)"
              >
                <Icon icon="mdi:checkbox-multiple-marked-outline" class="h-[18px] w-[18px]" />
              </button>
            </div>
          </div>
          <div 
          ...
          >
            ...
            <template v-else>
              <div class="flex flex-col gap-2">
                ...
                <MeetingChatCard v-if="message.meetingId" :meeting-id="message.meetingId" class="mt-1" />
              </div>
            </template>
            <p
            ...
            >
              ...
              <button
                type="button"
                class="inline-flex h-8 w-8 items-center justify-center rounded-md text-gray-400 outline-none hover:bg-gray-300/70 hover:text-gray-700 focus-visible:ring-2 focus-visible:ring-gray-400 dark:text-slate-500 dark:hover:bg-slate-700 dark:hover:text-slate-300"
                :title="t('common.messageBatchDelete')"
                :aria-label="t('common.messageBatchDelete')"
                @click="enterBatchMode(message.id)"
              >
...
import type { MessageAttachment, MessageProps } from '../../shared/types'
import { looksLikeImageGenerationPrompt } from '../../domains/media/imageGenPromptDetect'
import { markdownToPlainText } from '../../domains/chat/markdownPlain'
import { useMessageStore } from '../../domains/chat/stores/message'
import { buildPptxForAnswerMessage } from '../../domains/media/ppt/pptExportRenderer'
import WebSearchReferencesBar from './WebSearchReferencesBar.vue'
import MeetingChatCard from '../../domains/meeting/components/MeetingChatCard.vue'
...
function enterBatchMode(preSelectId?: number) {
  batchMode.value = true
  batchSelectedIds.value = preSelectId != null ? new Set([preSelectId]) : new Set()
}
...
function toggleBatchSelectAll() {
  if (batchSelectedIds.value.size === props.messages.length) {
    batchSelectedIds.value = new Set()
  } else {
    batchSelectedIds.value = new Set(props.messages.map((m) => m.id))
  }
}

async function onDeleteMessage(id: number) {
  if (!window.confirm(t('common.messageDeleteConfirm'))) return
...
async function confirmBatchDelete() {
  ...
  if (!window.confirm(t('common.messageBatchDeleteConfirm', { count: ids.length }))) return
  await messageStore.deleteManyMessages(ids)
  exitBatchMode()
}
...
/** 当前正在朗读的助手消息 id；与 petSpeak / speechSynthesis 同步 */
const speakingMessageId = ref<number | null>(null)
let preferNailongSpeak = true
let unsubSpeakConfig: (() => void) | null = null

async function refreshSpeakPrefs() {
  try {
    const cfg = await window.electronAPI.getConfig()
    preferNailongSpeak = cfg.nailongTtsEnabled !== false || Boolean(cfg.desktopPetEnabled)
  } catch {
    preferNailongSpeak = true
  }
}
void refreshSpeakPrefs()
unsubSpeakConfig = window.electronAPI.onConfigChanged(() => {
  void refreshSpeakPrefs()
})
...
function isAnswerVisuallyEmpty(m: MessageProps): boolean {
  ...
  if (m.imagePath || m.exportPptPath || m.meetingId) return false
...
async function speakAnswer(m: MessageProps) {
  ...
  if (preferNailongSpeak) {
    try {
      const r = await window.electronAPI.petSpeakText({ text: text.slice(0, 300), rate: 2 })
      if (speakingMessageId.value === mid) {
        speakingMessageId.value = null
      }
      if (r?.cancelled) return
      if (r?.ok && r.engine === 'nailong') return
      if (r?.ok && r.engine === 'system') {
        /* 已用系统音播完，不再叠一层 speechSynthesis */
        return
      }
    } catch {
      /* fall through to browser TTS */
    }
    if (speakingMessageId.value !== mid) return
  }
