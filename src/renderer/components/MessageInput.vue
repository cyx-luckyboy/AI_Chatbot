<template>
  ...
  >
    ...
    <div v-if="agentMode" class="mb-2 flex flex-wrap items-center gap-2 px-0.5">
      <div class="inline-flex items-center gap-1.5 rounded-md bg-emerald-100 px-2 py-1.5 pl-2.5 text-sm text-emerald-950 dark:bg-emerald-950/50 dark:text-emerald-100">
        <Icon icon="mdi:desktop-classic" class="h-4 w-4 shrink-0" />
        <span class="font-medium">{{ t('common.agentTag') }}</span>
        <button
          type="button"
          class="ml-0.5 flex h-5 w-5 items-center justify-center rounded hover:bg-emerald-200/80 dark:hover:bg-emerald-900/60"
          :title="t('common.agentClose')"
          :aria-label="t('common.agentClose')"
          @click="disableAgentMode"
        >
          <Icon icon="radix-icons:cross-2" width="14" height="14" />
        </button>
      </div>
      <button
        type="button"
        class="inline-flex max-w-full items-center gap-1 rounded-md bg-gray-100 px-2 py-1.5 text-xs text-gray-800 outline-none hover:bg-gray-200 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700"
        :disabled="disabled && !canStop"
        @click="pickAgentWorkspace"
      >
        <Icon icon="mdi:folder-outline" class="h-3.5 w-3.5 shrink-0" />
        <span class="truncate" :title="agentWorkspacePath || undefined">
          {{ agentWorkspacePath ? `${t('common.agentWorkspaceLabel')}: ${agentWorkspaceShort}` : t('common.agentPickWorkspace') }}
        </span>
      </button>
      <span class="text-xs text-emerald-800/90 dark:text-emerald-200/80">{{ t('common.agentHint') }}</span>
    </div>
    ...
    <div v-if="imageGenMode" class="flex flex-col gap-2 px-0.5">
      ...
      <div class="flex flex-wrap items-center gap-1.5">
        ...
        <button
          v-else
          type="button"
          class="inline-flex h-9 shrink-0 items-center gap-1.5 rounded-md bg-red-600 px-3 text-sm font-medium text-white outline-none hover:bg-red-700 focus-visible:ring-2 focus-visible:ring-red-300"
          :title="t('common.stopGeneratingTitle')"
          :aria-label="t('common.stopGeneratingTitle')"
          @click="emit('stop')"
        >
    ...
    <div v-else class="flex items-end gap-2">
      <DropdownMenuRoot v-if="!translateMode && !pptGenMode && !webSearchMode && !agentMode">
        <DropdownMenuTrigger
          type="button"
        ...
        <DropdownMenuPortal>
          ...
          >
            ...
            <DropdownMenuItem
              class="flex cursor-default select-none items-center gap-2.5 rounded-md px-3 py-2.5 text-sm text-gray-800 outline-none data-[highlighted]:bg-gray-100 dark:text-slate-200 dark:data-[highlighted]:bg-slate-800"
              @select="enableAgentMode"
            >
              <Icon icon="mdi:desktop-classic" width="20" height="20" class="shrink-0 text-gray-500" />
              <span>{{ t('common.agentTag') }}</span>
            </DropdownMenuItem>
            <DropdownMenuItem
              class="flex cursor-default select-none items-center gap-2.5 rounded-md px-3 py-2.5 text-sm text-gray-800 outline-none data-[highlighted]:bg-gray-100 dark:text-slate-200 dark:data-[highlighted]:bg-slate-800"
              @select="openMeetingsEntry"
            >
              <Icon icon="mdi:microphone" width="20" height="20" class="shrink-0 text-gray-500" />
              <span>{{ t('common.meetingRecordTag') }}</span>
            </DropdownMenuItem>
      ...
      <button
        v-else
        type="button"
        class="inline-flex h-10 shrink-0 items-center gap-1.5 rounded-md bg-red-600 px-3.5 text-sm font-medium text-white outline-none hover:bg-red-700 focus-visible:ring-2 focus-visible:ring-red-300"
        :title="t('common.stopGeneratingTitle')"
        :aria-label="t('common.stopGeneratingTitle')"
        @click="emit('stop')"
      >
    ...
    <div
      v-if="!translateMode && !pptGenMode && !imageGenMode && !webSearchMode && !agentMode"
...
import { useVoiceInput } from '../../domains/speech/useVoiceInput'
...
} from '../../domains/media/jimengModels'
import {
  MAX_IMPORT_ATTACHMENT_BYTES,
  MAX_INLINE_ATTACHMENT_BYTES,
} from '../../domains/chat/attachmentLimits'
import type {
  ImageGenSizeId,
...
const inputPlaceholder = computed(() => {
  ...
  if (agentMode.value) return t('common.agentPlaceholder')
  if (pptGenMode.value) return t('common.pptGenPlaceholder')
  if (imageGenMode.value) return t('common.imageGenPlaceholder')
  return undefined
})

const agentWorkspaceShort = computed(() => {
  const p = agentWorkspacePath.value
  if (!p) return ''
...
onMounted(() => {
  void (async () => {
    try {
      ...
      agentWorkspacePath.value = (cfg.agentWorkspacePath || '').trim()
...
function enableAgentMode() {
  exitSpecialModes()
  agentMode.value = true
  void persistAgentMode(true)
  void refreshAgentWorkspace()
  void nextTick(() => textareaRef.value?.focus())
}

async function persistAgentMode(on: boolean) {
  try {
    await window.electronAPI.updateConfig({ agentModeEnabled: on })
  } catch (e) {
    console.error('persistAgentMode', e)
  }
}
...
async function refreshAgentWorkspace() {
  try {
    ...
    agentWorkspacePath.value = ''
  }
}

async function pickAgentWorkspace() {
  if (props.disabled && !props.canStop) return
  try {
    const res = await window.electronAPI.pickAgentWorkspace()
    if (res.ok) agentWorkspacePath.value = res.path
  } catch (e) {
    console.error('pickAgentWorkspace', e)
  }
}
