...
export const useConversationStore = defineStore('conversation', {
  ...
  actions: {
    ...
    async renameConversation(id: number, title: string, opts?: { aiGenerated?: boolean }) {
      const trimmed = title.trim()
      if (!trimmed) return
      const now = new Date().toISOString()
      const patch: Partial<ConversationProps> = { title: trimmed, updatedAt: now }
      if (opts?.aiGenerated) patch.titleAiGenerated = true
      await db.conversations.update(id, patch)
      const item = this.items.find((c) => c.id === id)
      if (item) {
        item.title = trimmed
        item.updatedAt = now
        if (opts?.aiGenerated) item.titleAiGenerated = true
      }
    },
    ...
    async updateConversationModel(id: number, providerId: number, selectedModel: string) {
      const model = selectedModel.trim()
      if (!model || !Number.isFinite(providerId)) return
      const now = new Date().toISOString()
      await db.conversations.update(id, { providerId, selectedModel: model, updatedAt: now })
      const item = this.items.find((c) => c.id === id)
      if (item) {
