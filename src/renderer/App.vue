    ...
    >
      ...
      <div v-show="!sidebarCollapsed && !showSearchBrowser" class="min-h-0 flex-1 overflow-y-auto">
        <ConversationList
          ...
          :items="filteredItems"
          :batch-mode="batchDeleteMode"
          @update:selected-ids="onBatchSelectedIdsUpdate"
        />
      </div>
      <div v-show="!sidebarCollapsed" class="shrink-0 border-t border-gray-300 p-2 dark:border-slate-600">
        <template v-if="batchDeleteMode">
          <div class="mb-2 flex items-center justify-between text-xs text-gray-500 dark:text-slate-400">
            <span>{{ t('common.conversationBatchDeleteSelected', { count: batchSelectedIds.size }) }}</span>
            <button
              type="button"
              class="text-blue-600 hover:underline dark:text-blue-400"
              @click="toggleBatchSelectAll"
            >
              {{ batchSelectedIds.size === filteredItems.length ? t('common.conversationBatchDeleteDeselectAll') : t('common.conversationBatchDeleteSelectAll') }}
            </button>
          </div>
          <div class="grid grid-cols-2 gap-2">
            <Button plain class="w-full" type="button" @click="exitBatchDeleteMode">
              {{ t('common.conversationBatchDeleteCancel') }}
            </Button>
            <button
              type="button"
              class="inline-flex w-full items-center justify-center gap-1.5 rounded-md bg-red-600 px-3 py-1.5 text-sm font-medium text-white shadow-sm transition-colors hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-red-700 dark:hover:bg-red-600"
              :disabled="batchSelectedIds.size === 0"
              @click="confirmBatchDelete"
            >
              <Icon icon="radix-icons:trash" class="h-4 w-4" />
              {{ t('common.conversationBatchDeleteConfirmBtn') }}
              <span v-if="batchSelectedIds.size > 0">({{ batchSelectedIds.size }})</span>
            </button>
          </div>
        </template>
        <template v-else>
          ...
          <RouterLink to="/workspace" custom v-slot="{ navigate }">
            <button
              type="button"
              class="mt-2 inline-flex w-full items-center justify-center gap-1.5 rounded-md border border-emerald-300 bg-emerald-50 px-3 py-1.5 text-sm font-medium text-emerald-800 shadow-sm transition-colors hover:bg-emerald-100 dark:border-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-200 dark:hover:bg-emerald-950/70"
              @click="navigate"
            >
              <Icon icon="mdi:desktop-classic" class="h-4 w-4" />
              {{ t('workspace.title') }}
            </button>
          </RouterLink>
          <button
            type="button"
            class="mt-2 inline-flex w-full items-center justify-center gap-1.5 rounded-md border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-red-600 shadow-sm transition-colors hover:bg-red-50 dark:border-slate-600 dark:bg-slate-800 dark:text-red-400 dark:hover:bg-red-950/30"
            @click="enterBatchDeleteMode"
          >
            <Icon icon="radix-icons:trash" class="h-4 w-4" />
            {{ t('common.conversationBatchDelete') }}
          </button>
        </template>
      </div>
    </div>
...
import { initI18n } from '../shared/i18n/index'
import { db, initProviders } from '../shared/db'
...
import LocationBadge from '../domains/location/LocationBadge.vue'
...
import { setupMeetingGlobalHandlers } from '../domains/meeting/meetingGlobalHandlers'
import { setupPetChatGlobalHandlers } from '../domains/pet/petChatHandlers'
import { useMeetingStore } from '../domains/meeting/stores/meeting'

const SIDEBAR_COLLAPSED_KEY = 'vchat-sidebar-collapsed'

type ConversationListInstance = InstanceType<typeof ConversationList>
...
const conversationListRef = ref<ConversationListInstance | null>(null)
...
/** 日常会话列表：排除工作台 Agent / 桌宠会话 */
const dailyConversationItems = computed(() =>
  conversationStore.sortedItems.filter((c) => {
...
const filteredItems = computed(() => {
  const q = conversationSearchQuery.value.trim().toLowerCase()
  if (!q) return dailyConversationItems.value
  const fromMessages = messageMatchedConversationIds.value
  return dailyConversationItems.value.filter((c) => {
    const title = (c.title ?? '').toLowerCase()
    const model = (c.selectedModel ?? '').toLowerCase()
...
function enterBatchDeleteMode() {
  ...
  conversationListRef.value?.clearSelection()
}
...
function onBatchSelectedIdsUpdate(ids: Set<number>) {
  batchSelectedIds.value = new Set(ids)
}

function toggleBatchSelectAll() {
  if (batchSelectedIds.value.size === filteredItems.value.length) {
    conversationListRef.value?.deselectAll()
  } else {
    conversationListRef.value?.selectAll()
  }
}
...
async function confirmBatchDelete() {
  ...
  const confirmed = window.confirm(
    t('common.conversationBatchDeleteConfirm', { count: ids.length }),
  )
  if (!confirmed) return
  for (const id of ids) {
    await conversationStore.deleteConversation(id)
  }
  if (ids.includes(conversationStore.selectedId)) {
...
let offMeetingGlobal: (() => void) | undefined
let offPetChatGlobal: (() => void) | undefined
...
onMounted(async () => {
  ...
  offMeetingGlobal = setupMeetingGlobalHandlers()
  offPetChatGlobal = setupPetChatGlobalHandlers(router)
